...
fn launch_editor(graph: Graph) {
    use bevy::prelude::*;
    use pipe_graph::systems::{GraphResource, PipeGraphEditorPlugin};
...
fn launch_editor(_graph: Graph) {
    eprintln!(
        "❌ Error: The editor requires the `bevy` feature. Re-run with `cargo run --features bevy -- edit <file>`."
    );
    std::process::exit(1);
}
    ...
    match &cli.command {
        ...
        Commands::Edit { file } => {
            ...
            let graph = match load_graph(file) {
                ...
                Err(e) => {
                    eprintln!(
                        "Warning: failed to load graph ({}), starting with an empty graph.",
                        e
                    );
                    Graph::new()
                }
